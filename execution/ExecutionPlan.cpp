...
bool ExecutionPlan::ResultInfo::add(const ResultInfo& result,
		DBDataType type)
{
	if (m_bNull)
	{
...
int ExecutionPlan::ResultInfo::compare(const ParseNode* pValue, DBDataType type)
{
	if (m_bNull)
	{
	...
	case TYPE_STRING: {
		...
		return string_compare(m_value.m_pszResult, m_len, pValue->m_pszValue, pValue->m_iValue);
	}
		//case TYPE_DATE:
		//case TYPE_DATETIME:
		//case TYPE_DOUBLE:
