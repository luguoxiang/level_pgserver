...
int64_t parseTime(const char* pszTime)
{
	...
	if (iYear - 1900 != time.tm_year || iMonth - 1 != time.tm_mon || iDay != time.tm_mday) 
	{
        	return 0; 
	}
...
char* memPoolAlloc(size_t iSize, ParseResult *p)
{
	...
	else
	{
		...
		{
			pBlock->m_pNext = (MemoryPoolBlock*) malloc(
					sizeof(MemoryPoolBlock));
...
size_t parseString(const char* pszSrc, char* pszDest, size_t iLen)
{
	...
	for(i = 0; i < iLen; ++i)
	{
		...
		if(c == '\\')
		{
			if(i + 3 < iLen &&  IS_DIGIT(pszSrc[i + 1]) && IS_DIGIT(pszSrc[i + 2]) && IS_DIGIT(pszSrc[i + 3]))
			{
				c = (pszSrc[++i] - '0') * 64;
				c += (pszSrc[++i] - '0') * 8;
				c += pszSrc[++i] - '0';
