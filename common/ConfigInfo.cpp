...
void TableInfo::addColumn(MetaConfig* pConfig, const char* pszValue)
{
	...
	if (iLen <= 0)
	{
		...
		case TYPE_DATE:
			...
			break;
		case TYPE_DOUBLE:
			throw new ConfigException("Missing precision for type %s ", s.c_str());
		case TYPE_STRING:
			pColumn->m_iLen = -1;
			break;
		default:
...
void TableInfo::addKeyColumn(std::string& name)
{
	...
	{
		throw new ConfigException("rowkey column with double type is not supported");
	}
	pColumn->m_iKeyIndex = m_keys.size();
	m_keys.push_back(pColumn);
}

void TableInfo::getDBColumns(ParseNode* pColumn, std::vector<DBColumnInfo*>& columns)
{
	if (pColumn == 0)
	{
	...
	else
	{
		...
		for (size_t j = 0; j < pColumn->m_iChildNum; ++j)
		{
			...
			{
				throw new ParseException("Unsupported select expression:", p->m_pszExpr);
			}
			DBColumnInfo* pColumnInfo = getColumnByName(p->m_pszValue);
			if (pColumnInfo == 0)
			{
				throw new ParseException("Table %s does not have column named %s!",
						getName(), p->m_pszValue);
			}
